...
The cpu is responsible for setting the shaders and resources for each stage.
The \href{https://gist.github.com/HatecrewJP/04172ef23d3d41105c84c74415379846}{vertex and index data}, passed by the cpu, describes 8 vertices and 12 triangles, which describe the cube.


\subsection{Input Assembler Stage}
The input assembler(IA) stage is responsible for transforming the vertices and indices into primitives, that can be processed. Primives can be points, lines or triangles.
...
To see an image from the data that has been passed to the cpu, a few pipeline stages must be active\cite{HLSLVS}.
In the cube example input assembler, vertex shader, rasterizer and pixel shader are active. The shaders only pass the data to the next stage without modifying it.

\noindent This is the output with the IA stage and only passing through data.
