...
Following this, are explanations of HLSL concepts including data types, semantics, flow control and shader models.
...
integer, signed and unsigned, floating points or bools are available. There are also less common types like snorm float or unorm float. 
These are normalized floating point numbers. A snorm float can take any value from the intervall [-1,1] and unorm float can take values in the intervall [0,1].
...
The return value of the main function is, what is passed to the next stage. If a single value is required by the following stage, it's sufficient to simply return this value.
Everytime more than a single value is required, the function has to return a struct with the data.
...
It takes a patch as input and outputs control points, which are used by the domain shader, and patch-constant data, which is used by the tessllator.
...
By interpolating betweeen control points with the UV(W)-values, new vertices can be created.
...
The hull shader nees to know which shape of the patch, how many control points it should output and which function is used to calculate the patch-constant data.
...
\noindent The geometry shaders way of outputting data is different than the other stages. 
...
\noindent Notice, that the ColorX variables are from the constant buffer and can be used like global variables.
