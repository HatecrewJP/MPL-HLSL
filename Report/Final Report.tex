...
integer, signed and unsigned, floating points or bools are available. There are also less common types like snorm or unorm. 
These are normalized floating point numbers. A snorm can take any value from the intervall [-1,1] and unorm can take values in the intervall [0,1].
Unorm is often used to display colors.

\noindent \textit{Example:}
\begin{minted}[xleftmargin=0.3cm,bgcolor=bg]{hlsl}
uint   AnUnsignedInteger;
double ADoublePrecisionFloatingPoint;
unorm  AnUnsignedNormalizedFloatingPoint;
\end{minted}
...
\subsection{Buffers}
A pipeline stage might need need more than the vertex data to calculate the output. This is where buffer objects are used. 
...
Structs are used to group data within a shader, whereas buffers containt data coming from the CPU.\@
