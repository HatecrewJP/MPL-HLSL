...
\section{Data Types}
HLSL supports various data types. Data can be stored in registers, which are made of four 32-bit components.
\textbf{Scalar} variables use one component and are able to represent different types such as integers, floats or booleans.
\textbf{Vectors} can use up to four components and use any of the scalar types. A \textbf{matrix} is a grid of up to four rows and colums, respectively.
The data type of matrices can be any of the scalar types. \textbf{Structs} are a collection of data and work like c's structs.
HLSL requires buffer objects that, which store data passed by the CPU. 
There are different types of buffer such as \textbf{cbuffer}-objects or \textbf{RWBuffer}.
A \textbf{cbuffer} is a buffer that contains constant data, whereas \textbf{RWBuffer} is a buffer with read and write access.
Each buffer has its own way to access the data. The identifiers in Constant Buffers can be accessed like global variables. 
RWBuffers can be accessed like an array.

\noindent \textit{Examples:}
\begin{minted}[xleftmargin=0.3cm,bgcolor=bg]{hlsl}
  float AScalarFloat;
  uint3 A3ComponentUnsignedIntegerVector;
  int2x2 A2by2SignedIntegerMatrix
  struct Coordinate2D{  //A struct representing
    float x;            //a 2D float Coordinate 
    float y;            
  }
  cbuffer Constants{  //A buffer that stores
    float Width;      //The height and width
    float Height;     //of a window made by the CPU
  }
\end{minted}
...
Since HLSL is a language for GPUs, and GPUs are made for Single Instruction Multiple Data (SIMD) and parallel execution,
...
\begin{itemize}
  ...
  \item \textbf{flatten}: Executes the \textit{true} and \textit{false} side of the if-statement and decides, which one use, based on the condition\cite{HLSLif}. 
\end{itemize}
...
Shader Models specify, which features can be used in shaders.
New Shader Models introduce new features or adjust existing features.
Each Shader Model specifies Shader Profiles\cite{HLSLShaderModel}, which are used by the compiler to determine which features can be used.
Every shader type has its own shader profile, available in different versions.

\noindent \textit{Example:}

Shader Model 4 introduced the shader profiles cs\_4\_0 and cs\_4\_1. Both profiles are for compute shaders.
...
\noindent \includegraphics[height= 0.2\textheight]{Assets/FinalImage.png}
...
The \href{https://gist.github.com/HatecrewJP/04172ef23d3d41105c84c74415379846}{vertex and index data}, passed by the CPU, describes 8 vertices and 12 triangles.
...
In the cube example the input assembler, vertex shader, rasterizer and pixel shader are active. The shaders are set to only pass the data to the next stage, without modifying it.

\subsection{Input Assembler Stage}
The input assembler stage(IA) receives the index and vertex buffers from the CPU. It then uses these buffers to create \href{https://en.wikipedia.org/wiki/Geometric_primitive}{primitves} (shapes).
Primitves are described by vertices. To find the vertices for a primitve, the IA reads consecutive indices from the index buffer. 
...
The vertex shader stage is the first stage after the input assembler stage and operates on the vertices of the primitives. It is responsible for placing the vertices at the correct position on the screen.
...
\noindent This is the stage, where the cube is being rotated and scaled down and then projected with an orthographic projection.
...
Without line 22, the cube would be streched along the x-axis. To compensate for that, the x coordinates must be divided by the aspect ratio (Width/Height).

\noindent Line 23 is interesting, because it is using an intrinsic. The \textbf{mul} intrinsic has to be used for efficient matrix multiplication.


\subsection{Tessellation Stages}
The tesselation stages enable hardware subdivision, which can be used to adjust the level of detail of a primitive.
...
\noindent Next is the \textbf{Tessellator Stage}, which cannot be programmed. It takes the patch generated by the hull shader and the patch-constant data as input 
and outputs the surface topology and UV(W)-coordinates to the domain stage\cite{HLSLTess}.


\noindent The \textbf{Domain Shader Stage} is responsible for calculating the positions of the new vertices based on the patch from the hull shader and the UV(W)-coordinates from the tessellator.
...
{
  DSOutput Output;
  Output.Position = UVWCoord.x * patch[0].Position + UVWCoord.y * patch[1].Position 
...
\noindent \includegraphics[height= 0.16\textheight]{Assets/Rasterizer.png}
