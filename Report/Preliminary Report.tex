...
\item\textbf{RWTexture2D}: A 2D read/write Texture Buffer Object
\end{itemize}


\noindent \textit{Buffer Examples:}
...
\noindent \textit{Examples:}
\begin{minted}[xleftmargin=0.3cm,bgcolor=bg]{hlsl}
//This is the struct from the previous example
struct ps_input{
  float4 vPosition : SV_Position;
  float4 Color : COLOR;
  float3 Normal : NORMAL;
...
\end{minted}
This data describes 8 vertices, each one with a different color.
...
\noindent \includegraphics[height= 0.2\textheight]{Assets/Tesselator.png}
