...
\begin{comment}
  ...
  -minted settings :xleftmargins = 0.3cm
\end{comment}
...
It is possible, that some concepts mentionend are not further explained or details are omitted, because they would go beyond the scope of this course. 
...
HLSL is based on c programming language\cite{DirectX2003} and adds additional concepts designed for gpu programming. 
...
\subsection{Vector}
Vector's are variables that use up to 4 components\cite{HLSLVector}. The type can be any of the scalar data types. A vector can be declared in two ways.
\vfill
\noindent Declaration with a template: 

\textbf{vector}<Type, Components = [1,4]>
...
\textbf{float[n]}, where n is a number in the intervall [1,4].
...
Accessing a component can be done by using the structure operator ``\textbf{.}''\cite{HLSLComponentMath} followd by the letters xyzw or rgba.
...
Accessing a component of a matrix can be done multiple ways using the structure operator \textbf{.}\cite{HLSLComponentMath}.

One way to index a component is \_mRC, where R and C correspond to Row/Column indices\cite{HLSLComponentMath}.
...
Most of the code is simple, but there are two interesting lines, line 18 and 19 and the uniform declared variables.

Variables declared as \textbf{uniform} have the same value for all invokations within a render pass and are constant.\cite{HLSLVars}
